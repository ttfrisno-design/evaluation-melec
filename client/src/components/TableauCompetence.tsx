 ...
 * Design: Dashboard Technique Compact — tableaux denses, accents couleur par compétence
 */
...
export default function TableauCompetence({ competence, notes, onNoteChange }: Props) {
  ...
  const { obtenu, max } = calculerNoteCompetence(competence, notes);
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ background: `${competence.couleur}0F`, borderBottom: collapsed ? "none" : `1px solid ${competence.couleur}30` }}
      ...
      >
        ...
        <div className="flex items-center gap-4 flex-shrink-0 ml-4">
          ...
          <div className="flex items-center gap-2">
            <div className="w-24 h-1.5 rounded-full overflow-hidden" style={{ background: "#F5F5F4" }}>
              <div
                className="h-full rounded-full transition-all duration-300"
                style={{ width: `${pourcentage}%`, background: getBarColor(pourcentage) }}
              />
            </div>
            <span className="text-sm font-semibold tabular-nums" style={{ color: competence.couleur, minWidth: "70px", textAlign: "right" }}>
              {obtenu} / {max}
            </span>
          </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr style={{ background: "#FAFAF9", borderBottom: "1px solid #E7E5E4" }}>
                <th className="text-left px-5 py-2.5 font-semibold text-xs uppercase tracking-wide" style={{ color: "#78716C", width: "60%" }}>
                ...
                <th className="text-center px-4 py-2.5 font-semibold text-xs uppercase tracking-wide" style={{ color: "#78716C", width: "10%" }}>
                  %
                </th>
              </tr>
            </thead>
            <tbody>
              {competence.criteres.map((critere, index) => {
                ...
                const pct = note !== null && note !== undefined ? Math.round((note / critere.noteMax) * 100) : null;
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-center">
                      <input
                        ...
                        style={{
                          background: note !== null && note !== undefined ? `${competence.couleur}10` : "#F5F5F4",
                          border: `1.5px solid ${note !== null && note !== undefined ? competence.couleur : "#E7E5E4"}`,
                          color: "#1C1917",
                        }}
                        onFocus={(e) => (e.target.style.borderColor = competence.couleur)}
                        onBlur={(e) => {
                          e.target.style.borderColor =
                            note !== null && note !== undefined ? competence.couleur : "#E7E5E4";
                        }}
                      />
                    </td>
                    <td className="px-4 py-3 text-center">
            ...
            {/* Pied de tableau — total compétence */}
            <tfoot>
              <tr style={{ background: `${competence.couleur}08`, borderTop: `2px solid ${competence.couleur}30` }}>
                <td className="px-5 py-2.5 text-sm font-bold" style={{ color: "#1C1917" }}>
                  Total {competence.code}
                </td>
                <td className="px-4 py-2.5 text-center text-sm font-bold" style={{ color: "#57534E" }}>
                  /{max}
                </td>
                <td className="px-4 py-2.5 text-center">
                  <span
                    className="text-sm font-bold tabular-nums"
                    style={{ color: competence.couleur }}
                  >
                    {obtenu}
                  </span>
                </td>
                <td className="px-4 py-2.5 text-center">
                  <span
                    ...
                    style={{ color: getBarColor(pourcentage) }}
                  >
